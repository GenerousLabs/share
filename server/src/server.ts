...
const INITIAL_BRANCH_MINIMUM_GIT_VERSION = "2.28.0";
...
const main = async () => {
  const gitVersion = await binaryVersion("git");

  const gitNeedsInitialBranch =
    compareVersions(gitVersion, INITIAL_BRANCH_MINIMUM_GIT_VERSION) >= 0;

  const initialBranch = gitNeedsInitialBranch ? "master" : undefined;
  ...
  app.post("/postoffice/:boxId", jsonParser, async (req, res) => {
    try {
      ...
      logger.error("Caught error in /postoffice/:boxId POST #1oBt7X", {
        error,
      });
      return res.status(500).end();
    }
