...
type Log = LogFunction & Record<LogLevel, LogLevelFunction>;
...
export const buildLogLine = ({
  ...
  const lineBase = [date, level, message];
...
export const _log: LogFunction = (level, message, meta) => {
...
export const log = ((): Log => {
  const logFn = _log as Log;
  logLevels.forEach((level) => {
    logFn[level] = _log.bind(null, level);
  });
  return logFn;
})();
